...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # it to an integer, and if ti's not, then we say its inivalid
...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.availabe_moves()) == 9:
            square.random.choice(game.availabe_moves()) # randomly choose one
        else:
    ...
    def minmax(self, state, player):
        ...
        other_player = 'O' if player == 'X' else 'X' # the other player.. so whatever letter is None
        ...
        for possible_move in state.availabe_moves():
            ...
            if player == max_player: # we are trying to maximaze the max_player
